...
IndexedModel OBJModel::ToIndexedModel()
{
    ...
    for(unsigned int i = 0; i < numIndices; i++)
    {
        ...
        if(it == normalModelIndexMap.end())
        {
            ...
            normalModelIndexMap.insert(std::pair<OBJIndex, unsigned int>(
                        *currentIndex, normalModelIndex));
        ...
        const OBJIndex* currentIndex, const IndexedModel& result)
{
    ...
    while(current != previous)
    {
        ...
        if(testIndex->vertexIndex == currentIndex->vertexIndex)
        {
            ...
            for(unsigned int i = countStart;
                    i < indexLookup.size() - countStart; i++)
            {
                OBJIndex* possibleIndex = indexLookup[current + i];
                     ...
                     possibleIndex->normalIndex == currentIndex->normalIndex))
                {
                    ...
                    for(unsigned int j = 0; j < result.positions.size(); j++)
                    {
                        if(currentPosition == result.positions[j]
                            && ((!hasUVs || currentTexCoord ==
                                    result.texCoords[j])
                            && (!hasNormals || currentNormal ==
                                result.normals[j])))
                        {
                            return j;
                        }
...
void OBJModel::CreateOBJFace(const std::string& line)
{
    ...
    this->OBJIndices.push_back(ParseOBJIndex(tokens[1], &this->hasUVs,
                &this->hasNormals));
    this->OBJIndices.push_back(ParseOBJIndex(tokens[2], &this->hasUVs,
                &this->hasNormals));
    this->OBJIndices.push_back(ParseOBJIndex(tokens[3], &this->hasUVs,
                &this->hasNormals));
    ...
    if((int)tokens.size() > 4)
    {
        ...
        this->OBJIndices.push_back(ParseOBJIndex(tokens[3], &this->hasUVs,
                    &this->hasNormals));
        this->OBJIndices.push_back(ParseOBJIndex(tokens[4], &this->hasUVs,
                    &this->hasNormals));
    }
}
        ...
        bool* hasNormals)
{
    ...
    unsigned int vertIndexEnd = FindNextChar(vertIndexStart, tokenString,
            tokenLength, '/');

    OBJIndex result;
    result.vertexIndex = ParseOBJIndexValue(token, vertIndexStart,
            vertIndexEnd);
    result.uvIndex = 0;
    ...
    *hasUVs = true;
    ...
    result.normalIndex = ParseOBJIndexValue(token, vertIndexStart,
            vertIndexEnd);
    *hasNormals = true;
